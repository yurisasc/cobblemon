...
import com.cablemc.pokemoncobbled.mod.config.CobbledConfig.exportSpawnsToConfig
...
object SpawnLoader {
        ...
        sets.addAll(
            files.mapNotNull {
                try {
                    ...
                    PokemonCobbledMod.LOGGER.error("Unable to load spawn set from ${it.pathString}")
    ...
    fun loadSetFromPath(path: Path): SpawnSet {
        val inputStream = PokemonCobbledMod::class.java.getResourceAsStream(path.toAbsolutePath().toString())
    ...
    fun loadPackedSets(path: Path): MutableList<SpawnSet> {
        val fileStream = PokemonCobbledMod::class.java.getResourceAsStream(path.toAbsolutePath().toString())
