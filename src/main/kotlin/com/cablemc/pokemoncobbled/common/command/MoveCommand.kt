...
object MoveCommand {
    ...
    fun register(dispatcher : CommandDispatcher<CommandSourceStack>) {
        val command = Commands.literal("move")
            .requires { it.hasPermission(4) }
            .then(
                Commands.argument("move", IntegerArgumentType.integer(1, 4))
                    .executes { execute(it) })
    ...
    private fun execute(context: CommandContext<CommandSourceStack>) : Int {
        ...
        if (entity is ServerPlayer && !entity.level.isClientSide) {
            ...
            if(battle != null) {
                ...
                if(actor != null)
                    battle.writeShowdownAction(">${actor.showdownId} move ${context.getArgument("move", Integer::class.java)}")
            } else {
