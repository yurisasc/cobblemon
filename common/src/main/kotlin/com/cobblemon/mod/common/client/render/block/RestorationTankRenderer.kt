...
class RestorationTankRenderer(ctx: BlockEntityRendererFactory.Context) : BlockEntityRenderer<RestorationTankBlockEntity> {
    override fun render(
        ...
        if (fillLevel == 0 && !struct.hasCreatePokemon) {
            return
        }

        if (struct.isRunning() or (struct.hasCreatePokemon)) renderFetus(entity, tickDelta, matrices, vertexConsumers, light, overlay)
        ...
        else if (struct.hasCreatePokemon) FLUID_MODELS[7]
