...
class MagikarpModel(root: ModelPart) : PokemonPoseableModel() {
    ...
    override fun registerPoses() {
        ...
        val sleepQuirk = quirk(secondsBetweenOccurrences = 60F to 120F) { bedrockStateful("magikarp", "ground_sleep_quirk")}
        val waterSurfaceQuirk = quirk(secondsBetweenOccurrences = 60F to 120F) { bedrockStateful("magikarp", "water_surface_quirk") }
        ...
        sleep = registerPose(
                ...
                idleAnimations = arrayOf(bedrock("magikarp", "ground_sleep"))
        )
        ...
        water_surface_idle = registerPose(
                ...
                idleAnimations = arrayOf(
                        bedrock("magikarp", "water_surface_idle"),
                ),
        ...
        water_surface_swim = registerPose(
                ...
                condition = { !it.isSubmergedInWater && it.isTouchingWater },
                idleAnimations = arrayOf(
                        bedrock("magikarp", "water_surface_swim"),
                ),
                transformedParts = arrayOf(
                        rootPart.createTransformation().addPosition(ModelPartTransformation.Y_AXIS, wateroffset)
                )
    ...
    ) = if (state.isPosedIn(standing, walk, battleidle, sleep)) bedrockStateful("magikarp", "ground_faint") else if (state.isPosedIn(water_surface_idle, water_surface_swim, watersleep )) bedrockStateful("magikarp", "water_faint") else null
}