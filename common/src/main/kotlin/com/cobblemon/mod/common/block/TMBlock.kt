...
class TMBlock(properties: Settings): Block(properties), Waterloggable {
    companion object {
        val PART = EnumProperty.of("part", TMPart::class.java)
    ...
    enum class TMPart(private val label: String) : StringIdentifiable {
        TOP("top"),
        BOTTOM("bottom");
    ...
    override fun getOutlineShape(blockState: BlockState, blockGetter: BlockView, blockPos: BlockPos, collisionContext: ShapeContext): VoxelShape {
        return if (blockState.get(PART) == TMPart.TOP)  {
    ...
    fun getPositionOfOtherPart(state: BlockState, pos: BlockPos): BlockPos {
        return if (state.get(PART) == TMPart.BOTTOM) {
            pos.up()
        } else {
            pos.down()
        }
    ...
    private fun isBase(state: BlockState): Boolean = state.get(PART) == TMPart.BOTTOM

    override fun onPlaced(world: World, pos: BlockPos, state: BlockState, placer: LivingEntity?, itemStack: ItemStack?) {
        world.setBlockState(pos.up(), state
            .with(PART, TMPart.TOP)
    ...
    override fun canPlaceAt(state: BlockState, world: WorldView, pos: BlockPos): Boolean {
        ...
        return if (state.get(PART) == TMPart.BOTTOM) blockState.isSideSolidFullSquare(world, blockPos, Direction.UP) else blockState.isOf(this)
    }
    ...
    override fun onBreak(world: World, pos: BlockPos, state: BlockState, player: PlayerEntity?) {
        if (!world.isClient && player?.isCreative == true) {
            ...
            if (part == TMPart.TOP && world.getBlockState(pos.down().also { blockPos = it }).also { blockState = it }.isOf(state.block) && blockState.get(PART) == TMPart.BOTTOM) {
    ...
    override fun getStateForNeighborUpdate(
        ...
        val isTM = neighborState.isOf(this)
        val part = state.get(PART)
        if (!isTM && part == TMPart.TOP && neighborPos == pos.down()) {
            return Blocks.AIR.defaultState
        } else if (!isTM && part == TMPart.BOTTOM && neighborPos == pos.up()) {
            return Blocks.AIR.defaultState
        }
