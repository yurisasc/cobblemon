...
package com.cobblemon.mod.common.pokemon.transformation.controller
...
import com.cobblemon.mod.common.api.pokemon.transformation.progress.TransformationProgressFactory
...
class ServerEvolutionController(override val pokemon: Pokemon) : EvolutionController<Evolution> {
    ...
    private val progress = arrayListOf<TransformationProgress<*>>()
    ...
    override fun start(evolution: Evolution) {
        CobblemonEvents.EVOLUTION_ACCEPTED.postThen(
            ...
            ifSucceeded = {
                ...
                evolution.forceStart(this.pokemon)
            }
        )
    }

    override fun progress(): Collection<TransformationProgress<*>> = this.progress.toList()

    override fun <P : TransformationProgress<*>> trackProgress(progress: P): P {
        this.progress.add(progress)
        return progress
    }

    override fun <P : TransformationProgress<*>> progressFirstOrCreate(predicate: (progress: TransformationProgress<*>) -> Boolean, progressFactory: () -> P): P {
    ...
    override fun loadFromNBT(nbt: NbtElement) {
        ...
        for (tag in progressList.filterIsInstance<NbtCompound>()) {
            TransformationProgressFactory.create(tag.getString(ID))?.let { progress ->
    ...
    override fun loadFromJson(json: JsonElement) {
        ...
        for (element in progressArray) {
            ...
            TransformationProgressFactory.create(jObject.get(ID).asString)?.let { progress ->
