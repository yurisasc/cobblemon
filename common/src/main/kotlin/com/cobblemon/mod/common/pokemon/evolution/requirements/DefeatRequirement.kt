...
class DefeatRequirement(val target: PokemonProperties, val amount: Int) : EvolutionRequirement {
    ...
    companion object {
        ...
        val CODEC: Codec<DefeatRequirement> = RecordCodecBuilder.create { builder ->
            builder.group(
                PokemonProperties.CODEC.fieldOf("target").forGetter(DefeatRequirement::target),
                Codec.INT.fieldOf("amount").forGetter(DefeatRequirement::amount)
            ).apply(builder, ::DefeatRequirement)
        }

        internal val VARIANT: Variant<EvolutionRequirement> = Variant(cobblemonResource("defeat"), CODEC)
