...
package com.cablemc.pokemod.common.client.gui
...
class PartyOverlay : InGameHud(MinecraftClient.getInstance(), MinecraftClient.getInstance().itemRenderer) {
    ...
    val partySlot = pokemodResource("ui/party/party_slot.png")
    val partySlotActive = pokemodResource("ui/party/party_slot_active.png")
    val partySlotFainted = pokemodResource("ui/party/party_slot_fainted.png")
    val partySlotFaintedActive = pokemodResource("ui/party/party_slot_fainted_active.png")
    val partySlotCollapsed = pokemodResource("ui/party/party_slot_collapsed.png")
    val genderIconMale = pokemodResource("ui/party/party_gender_male.png")
    val genderIconFemale = pokemodResource("ui/party/party_gender_female.png")
    val portraitBackground = pokemodResource("ui/party/party_slot_portrait_background.png")
    ...
    override fun render(matrixStack: MatrixStack, partialDeltaTicks: Float) {
        ...
        val panelX = 0
        val party = PokemodClient.storage.myParty
        if (party.slots.none { it != null }) {
            if (PokemodClient.clientPlayerData.promptStarter &&
                !PokemodClient.clientPlayerData.starterLocked &&
                !PokemodClient.clientPlayerData.starterSelected &&
                !PokemodClient.checkedStarterScreen
            ) {
                drawScaledText(
        ...
        val frameOffsetX = 17
        val frameOffsetY = 2
        val selectedSlot = PokemodClient.storage.selectedSlot
        ...
        party.slots.forEachIndexed { index, pokemon ->
            ...
            if (pokemon != null) {
                ...
                val ballIcon = pokemodResource("ui/ball/" + pokemon.caughtBall.name.path + ".png")
                val ballHeight = 22
                blitk(
                    matrixStack = matrixStack,
                    texture = ballIcon,
