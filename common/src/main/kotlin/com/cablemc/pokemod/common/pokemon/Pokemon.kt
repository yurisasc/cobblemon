...
import com.cablemc.pokemod.common.PokemodNetwork.sendToPlayers
import com.cablemc.pokemod.common.PokemodSounds
import com.cablemc.pokemod.common.api.abilities.Abilities
import com.cablemc.pokemod.common.api.abilities.Ability
...
import com.cablemc.pokemod.common.pokemon.evolution.CobbledEvolutionProxy
...
open class Pokemon {
    ...
    var species = PokemonSpecies.random()
        set(value) {
            ...
            field = value
            val newFeatureKeys = species.features + Pokemod.config.globalFlagSpeciesFeatures + SpeciesFeature.globalFeatures().keys
    ...
    var currentHealth = this.hp
        set(value) {
            ...
            if (currentHealth <= 0 && value > 0) {
                this.healTimer = Pokemod.config.healTimer
            } else if (value <= 0) {
                entity?.health = 0F
            }
            field = min(hp, value)
            ...
            if (this.isFainted()) {
                ...
                val faintTime = Pokemod.config.defaultFaintTimer
    ...
    fun sendOutWithAnimation(source: LivingEntity, level: ServerWorld, position: Vec3d, battleId: UUID? = null, mutation: (PokemonEntity) -> Unit = {}): CompletableFuture<PokemonEntity> {
        ...
        sendOut(level, position) {
            level.playSoundServer(position, PokemodSounds.SEND_OUT.get(), volume = 0.2F)
    ...
    /**
     * A utility method that checks if this Pokémon species or form data contains the [PokemodPokemonLabels.LEGENDARY] label.
     * This is used in Pokémon officially considered legendary.
     *
     * @return If the Pokémon is legendary.
     */
    fun isLegendary() = this.hasLabels(PokemodPokemonLabels.LEGENDARY)

    /**
     * A utility method that checks if this Pokémon species or form data contains the [PokemodPokemonLabels.ULTRA_BEAST] label.
     * This is used in Pokémon officially considered legendary.
     *
     * @return If the Pokémon is an ultra beast.
     */
    fun isUltraBeast() = this.hasLabels(PokemodPokemonLabels.ULTRA_BEAST)

    /**
     * Checks if a Pokémon has all the given labels.
     * Tags used by the mod can be found in [PokemodPokemonLabels].
     *
    ...
    fun loadFromNBT(nbt: NbtCompound): Pokemon {
        ...
        try {
            val rawID = nbt.getString(DataKeys.POKEMON_SPECIES_IDENTIFIER).replace("pokemoncobbled", "pokemod")
    ...
    fun loadFromJSON(json: JsonObject): Pokemon {
        ...
        try {
            val rawID = json.get(DataKeys.POKEMON_SPECIES_IDENTIFIER).asString.replace("pokemoncobbled", "pokemod")
            species = PokemonSpecies.getByIdentifier(Identifier(rawID))
                ?: throw IllegalStateException("Failed to read a species with identifier $rawID")
        } catch (e: InvalidIdentifierException) {
    ...
    fun addExperience(source: ExperienceSource, xp: Int): AddExperienceResult {
        ...
        var appliedXP = xp
        PokemodEvents.EXPERIENCE_GAINED_EVENT_PRE.postThen(
        ...
        if (newLevel != oldLevel) {
            PokemodEvents.LEVEL_UP_EVENT.post(
