...
import com.cablemc.pokemod.common.util.AssetLoading.toPath
...
import com.cablemc.pokemod.common.util.isHigherVersion
...
object SpawnLoader {
    ...
    fun exportSet(folder: String, set: SpawnSet) {
        val file = File("./config/pokemod/spawning/spawns/$folder/${set.id.lowercase()}.set.json")
        file.parentFile.mkdirs()
        file.createNewFile()

        val internalStream = Pokemod::class.java.getResourceAsStream(set.path.toAbsolutePath().toString())!!
    ...
    fun getExternalSets(folder: String): MutableList<SpawnSet> {
        ...
        AssetLoading.searchFor(
            dir = "config/pokemod/spawning/spawns/$folder",
            suffix = ".json",
            list = files
        )
    ...
    fun loadSetFromPath(path: Path): SpawnSet {
        ...
        val inputStream = Pokemod.javaClass.getResourceAsStream(pathString)
    ...
    fun loadInternalFolder(folder: String): MutableList<SpawnSet> {
        val internalFolder = pokemodResource("spawning/spawns/$folder")
    ...
    fun loadPackedSets(path: Path): MutableList<SpawnSet> {
        val fileStream = Pokemod::class.java.getResourceAsStream(path.toAbsolutePath().toString())
