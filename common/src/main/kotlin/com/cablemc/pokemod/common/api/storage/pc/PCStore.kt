...
package com.cablemc.pokemod.common.api.storage.pc

import com.cablemc.pokemod.common.Pokemod
import com.cablemc.pokemod.common.api.reactive.SimpleObservable
import com.cablemc.pokemod.common.api.storage.BottomlessStore
...
open class PCStore(
    ...
    override fun loadFromNBT(nbt: NbtCompound): PokemonStore<PCPosition> {
        ...
        if (!lockedSize && boxes.size != Pokemod.config.defaultBoxCount) {
            resize(Pokemod.config.defaultBoxCount, lockNewSize = false)
        } else {
            tryRestoreBackedUpPokemon()
        }
        return this
    ...
    override fun loadFromJSON(json: JsonObject): PokemonStore<PCPosition> {
        ...
        if (!lockedSize && boxes.size != Pokemod.config.defaultBoxCount) {
            resize(newSize = Pokemod.config.defaultBoxCount, lockNewSize = false)
        } else {
            tryRestoreBackedUpPokemon()
        }
        return this
