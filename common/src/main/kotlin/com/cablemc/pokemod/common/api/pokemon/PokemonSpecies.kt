...
import com.cablemc.pokemod.common.api.entity.EntityDimensionsAdapter
import com.cablemc.pokemod.common.api.moves.MoveTemplate
import com.cablemc.pokemod.common.api.moves.adapters.MoveTemplateAdapter
import com.cablemc.pokemod.common.api.pokemon.effect.ShoulderEffect
import com.cablemc.pokemod.common.api.pokemon.effect.adapter.ShoulderEffectAdapter
...
object PokemonSpecies : JsonDataRegistry<Species> {
    ...
    override val id = pokemodResource("species")
    ...
    private const val DUMMY_SPECIES_KEY = "${Pokemod.MODID}dummy"
    private const val DUMMY_SPECIES_NAME = "${Pokemod.MODID}:Dummy"
    ...
    object SpeciesByNameDelegate {
        operator fun getValue(_species: PokemonSpecies, property: KProperty<*>) = getByIdentifier(pokemodResource(property.name.lowercase()))
    }
     ...
     * This uses [getByIdentifier] using the [Pokemod.MODID] as the namespace and the [name] as the path.
     *
     * @param name The path of the species asset.
     * @return The [Species] if existing.
     */
    fun getByName(name: String) = this.getByIdentifier(pokemodResource(name))
     ...
     * @return The [Species] if existing.
     */
    fun getByPokedexNumber(ndex: Int, namespace: String = Pokemod.MODID) = this.speciesByDex.get(namespace, ndex)
    ...
    private fun createShowdownData() {
        Pokemod.LOGGER.info("Creating showdown data for species")
        ...
        Pokemod.LOGGER.info("Finished creating showdown data for species")
    }
