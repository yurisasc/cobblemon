...
import com.cablemc.pokemod.common.battles.BattleTypes
...
import com.cablemc.pokemod.common.util.sendParticlesServer
...
class EmptyPokeBallEntity(
    ...
    world: World,
    entityType: EntityType<out EmptyPokeBallEntity> = com.cablemc.pokemod.common.PokemodEntities.EMPTY_POKEBALL_TYPE
    ...
    val delegate = if (world.isClient) {
        com.cablemc.pokemod.common.client.entity.EmptyPokeBallClientDelegate()
    } else {
                    ...
                    if (countMovable > throwerActor.expectingCaptureActions) {
                        ...
                        battle.broadcastChatMessage(
                            lang(
                                ...
                                PokemodItems.ballMap[pokeBall]!!.get().name,
    ...
    override fun tick() {
        ...
        if (world.isServerSide()) {
            ...
            if (isOnGround && captureState.get() == CaptureState.FALL.ordinal.toByte()) {
                ...
                val captureResult = Pokemod.captureCalculator.processCapture(owner as ServerPlayerEntity, pokeBall, capturingPokemon!!.pokemon, null)
                    ...
                    .execute {
                        ...
                        if (rollsRemaining <= 0) {
                            if (captureResult.isSuccessfulCapture) {
                                ...
                                world.playSoundServer(pos, PokemodSounds.CAPTURE_SUCCEEDED.get(), volume = 0.3F, pitch = 1F)
                        ...
                        rollsRemaining--
                        world.playSoundServer(pos, PokemodSounds.POKEBALL_SHAKE.get())
    ...
    override fun getDefaultItem(): Item = PokemodItems.ballMap[pokeBall]?.get() as Item
    ...
    private fun attemptCatch(pokemonEntity: PokemonEntity) {
        ...
        world.playSoundServer(pos, PokemodSounds.POKEBALL_HIT.get())
        ...
        afterOnMain(seconds = 0.7F) {
            velocity = Vec3d.ZERO
            setNoGravity(true)
            world.playSoundServer(pos, PokemodSounds.CAPTURE_STARTED.get(), volume = 0.2F)
