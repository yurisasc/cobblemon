...
class MagbyModel(root: ModelPart) : PokemonPoseableModel(), HeadedFrame {
    ...
    override fun registerPoses() {
        standing = registerPose(
            ...
            idleAnimations = arrayOf(
                singleBoneLook()
                // bedrock("0240_magby/magby", "ground_idle")
            )
        )
        ...
        walk = registerPose(
            ...
            idleAnimations = arrayOf(
                singleBoneLook()
                // bedrock("0240_magby/magby", "ground_walk")
            )
        )
    }
...
//    ) = if (state.isPosedIn(standing, walk)) bedrockStateful("0240_magby/magby", "faint") else null
}